...
# manually add a backend machine
resource "azurerm_linux_virtual_machine" "backend_vm" {
  name                            = "backend_${var.sysadmin_machine}"
  location                        = var.location
  computer_name                   = "backendVmWorker"
  admin_username      = "${var.TF_VAR_admin_username}"
  admin_password = "${var.TF_VAR_admin_password}"

  network_interface_ids           = [azurerm_network_interface.be_nic.id]
